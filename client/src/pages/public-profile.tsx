...
export default function PublicProfile() {
  ...
  useEffect(() => {
    if (!username || !data || likePromptDismissed) return;
    const seenKey = `hexed_seen_like_${username}`;
    if (localStorage.getItem(seenKey) === "1" || localStorage.getItem(alreadyLikedKey!) === "1") return;
    const t = setTimeout(() => {
      setShowLikePrompt(true);
    }, 15000);
    return () => clearTimeout(t);
  }, [username, data, likePromptDismissed]);
  ...
  return (
    ...
    >
      ...
      {/* Like count — top left (respects showLikeCounter setting) */}
      <AnimatePresence>
        {showLikeCounter && likeStatus && (
          <motion.div
            ...
            className="fixed top-4 left-4 z-50 flex items-center gap-1.5 px-3 py-1.5 rounded-full backdrop-blur-md border"
            style={{ backgroundColor: "rgba(0,0,0,0.65)", borderColor: themeColor + "44" }}
          >
            <Heart className="w-3 h-3 fill-current" style={{ color: themeColor }} />
                      ...
                      >
                        ...
                        {user.badges && user.badges.length > 0 && (
                          <TooltipProvider delayDuration={50}> {/* <--- CHANGE THIS LINE */}
                            ...
                            // Glow: shape-hugging drop-shadow
                            const glowDrop = badgesGlow
                              ? `drop-shadow(0 0 5px ${glowColor}dd) drop-shadow(0 0 10px ${glowColor}88)`
                              : "";

                            // Strong CSS filter that truly colors any badge (emoji or image) to match the theme color.
                            // Strategy: grayscale → sepia (base hue ~30°) → hue-rotate to target → saturate → brightness
                            const buildThemeFilter = () => {
                              if (themeS < 0.08) {
                                // Near-neutral (white/gray/black): strip color, set brightness
                                const bri = Math.max(0.25, themeL * 2.5);
                                return `grayscale(1) brightness(${bri.toFixed(2)})`;
                              }
                              const hueRot = themeH - 30; // sepia base is ~30°
                            ...
                            if (matchBadgesToTheme && isImg) {
                              imgFilter = [themeFilter, glowDrop].filter(Boolean).join(" ");
                            } else if (badgesGlow && isImg) {
                              imgFilter = glowDrop;
                            }

                            // For emoji: apply the same filter chain so they truly turn theme-colored
                            const emojiStyle: React.CSSProperties = (() => {
                              if (matchBadgesToTheme && badgesGlow) return { filter: `${themeFilter} ${glowDrop}`.trim() };
                              if (matchBadgesToTheme) return { filter: themeFilter };
