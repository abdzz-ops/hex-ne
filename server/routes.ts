...
import { users, profiles, badges, tracks, viewLogs, links, templates, discordEvents, tickets, ticketMessages, botSettings, announcements, changelogs, profileLikes, isStaff, canManageBadges, canGiveBadges, canAccessBotSettings, canManageTeam, canManageWebsite, canManageChangelogs, ROLE_HIERARCHY } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/public/like/:username", async (req, res) => {
    try {
      ...
      res.json({ liked: true, likes: Number(countRow?.count ?? 0) });
    } catch (err) {
